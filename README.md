MoP Watchdog GUI – Setup & Installation Guide
...
C:\MoPWatcher\ (Or simply place the files inside your “Repack” folder)
...
C:\MoPWatcher\
 ...
 ├─ watchdog.log
 ├─ MopWatcher.ico
 └─ stop_watchdog.txt  (created when stopping)
 
 2. Running the GUI
Right-click MoPWatcherGUI.ps1 → Run with PowerShell
—or—
...
•	Rounded dark/blue window
•	Custom title bar
•	Resizable autoscaling layout
•	Scrollable sections

 3. First-Time Configuration
Open the GUI → Fill out the three server paths:
 MySQL Script
...
2.	Enter your topic
    Example: mop-watchdog
3.	Choose which services should send notifications:
      o	MySQL
      o	Authserver
      o	Worldserver
...
5.	Click Test Notification to confirm delivery.
 
 Smart Notification Logic
    •	The system captures a baseline on startup
    •	A 2-second suppression avoids false "DOWN" messages when opening the GUI
    •	Only true state changes trigger alerts
...
Install Click: Install Service
This creates: MoPWatchdog as a Windows service configured to run the watchdog on boot.
...
The GUI includes a dedicated scrolling log window.
  •	Shows the contents of watchdog.log
  •	Updates once per second
  •	Independent scroll bar
  •	Automatically scrolls to end
